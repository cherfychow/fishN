...
ggplot() +
  theme_classic(base_size=13) +
  geom_line(data = SAC, aes(x = SACentry, y = spN, color = Camera), size=0.5, shape=21, alpha=0.5) +
...
for (i in 1:7) {
  ...
  SAC_curve[[i]] <- nls(formula = spN ~ a - (a-b) * exp(-c* SACentry), 
                        data = SAC[which(SAC$Camera == unique(SAC$Camera)[i]),] %>% as.data.frame(), 
                        start = list(a=20, b=0.2, c=0.01))
}

# errors for everything 4+, so try power curve
# a = scaling factor
# b = exponent rate
i = 4
  SAC_curve[[i]] <- nls(formula = spN ~ a + b * log(SACentry), 
                  data=SAC[which(SAC$Camera == unique(SAC$Camera)[i]),] %>% as.data.frame(), 
                  start = list(a = 1, b = 0.1))
  
i = 5
  SAC_curve[[i]] <- nls(formula = spN ~ a * SACentry ^ b, 
                  data=SAC[which(SAC$Camera == unique(SAC$Camera)[i]),] %>% as.data.frame(), 
...
for (i in 1:7) {
  SAC_pred[[i]] <- nlsBootPredict(nlsBoot(SAC_curve[[i]], niter=500), 
                                  newdata = data.frame(SACentry = seq(0, 2700, length.out = length(which(SAC$Camera == unique(SAC$Camera)[i])))), 
...
palette = cherulean_palettes[['gomphosus']]
for (i in 1:7) {
 plot_SACs <- plot_SACs +
    geom_ribbon(data=SAC_pred[[i]] %>% bind_cols(., SACentry = SAC$SACentry[which(SAC$Camera == unique(SAC$Camera)[i])]), 
                aes(ymin=lwr, ymax=upr, x=SACentry), fill=palette[i], alpha=0.2) +
    geom_line(data=data.frame(x=SAC$SACentry[which(SAC$Camera == unique(SAC$Camera)[i])], y=SAC_pred[[i]][,1]), aes(x, y), color=palette[i], size=0.5)
}
